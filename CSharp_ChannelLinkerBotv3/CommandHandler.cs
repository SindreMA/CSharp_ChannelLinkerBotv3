    ...
    class CommandHandler
    {
        ...
        public CommandHandler(DiscordSocketClient client)
        {
            ...
            try
            {
                ...
                /*
                foreach (var i in GuildSettingsList)
                {
                    var val = (int)i.LinkMode;
                    
                    if (val == 5)
                    {
                        i.LinkMode = (GuildSettingsDTO.Modes)0;
                    }
                    else
                    {
                        i.LinkMode = (GuildSettingsDTO.Modes)val + 1;
                    }
                }
                var s = JsonConvert.SerializeObject(GuildSettingsList);
                */
            }
            catch (Exception)
            {
        ...
        public bool CheckType(ulong fromChannel, ulong toChannel, SocketCommandContext Context)
        {
            ulong guildid = Context.Guild.Id;
            if (ChannelsLinkedList.Exists(x => x.GuildID == guildid && x.ChannelCopyTo == toChannel && x.ChannelCopyFrom == fromChannel))
            {

                var item = ChannelsLinkedList.Find(x => x.GuildID == guildid && x.ChannelCopyTo == toChannel && x.ChannelCopyFrom == fromChannel);
                if (item.LinkMode == ChannelLinkDTO.Modes.EmbedOnly)
                ...
                else if (item.LinkMode == ChannelLinkDTO.Modes.NoneEmbedOnly)
                {
                    if (Context.Message.Embeds.Count == 0)
                    {
                        return true;
                ...
                else if (item.LinkMode == ChannelLinkDTO.Modes.PicturesAndFilesOnly)
                {
                ...
                else if (item.LinkMode == ChannelLinkDTO.Modes.TextOnly)
                {
                ...
                else if (item.LinkMode == ChannelLinkDTO.Modes.FilesOnly)
                {
                    if (Context.Message.Attachments.Count != 0)
                    {
                        return true;
                ...
                else if (item.LinkMode == ChannelLinkDTO.Modes.All)
                {
        ...
        private async Task _client_MessageReceived(SocketMessage arg)
        {
            ...
            {
                var result = _service.ExecuteAsync(context, argPost,_services);
                                ...
                                if ((message.Embeds.Count != 0 && prefix.Prefix != null))
                                {
                                    foreach (var item in message.Embeds)
                                    {
                                        if (prefix.Prefix != null)
                                        {
                                            ...
                                            {
                                                if (CheckType(From.Id,To.Id,context))
                                                {
